...
import student.eg.gtalent_spring_boot_260801.constant.AuthOwnerTypes;
...
import student.eg.gtalent_spring_boot_260801.entity.PasswordResetToken;
import student.eg.gtalent_spring_boot_260801.entity.AuthToken;
import student.eg.gtalent_spring_boot_260801.entity.Member;
import student.eg.gtalent_spring_boot_260801.repository.AuthTokenRepository;
import student.eg.gtalent_spring_boot_260801.repository.PasswordResetTokenRepository;
import student.eg.gtalent_spring_boot_260801.repository.MemberRepository;
import student.eg.gtalent_spring_boot_260801.exception.MemberAccountException;
import student.eg.gtalent_spring_boot_260801.exception.ResourceNotFoundException;
...
public class MemberService {
    ...
    public Member register(MemberRegisterRequest request) {
        ...
        if (!request.getPassword().equals(request.getConfirmPassword())) {
            throw new MemberAccountException("confirmPassword", ResponseMessages.MEMBER_CONFIRM_PASSWORD_NOT_MATCH);
        }

        // 驗證輸入的帳號是否已存在系統
        String account = request.getAccount();
        if (this.repository.countByAccount(account) > 0) {
            throw new MemberAccountException("account", ResponseMessages.MEMBER_ACCOUNT_EXISTS);
        }
    ...
    public TokenResponse login(MemberLoginRequest request) {
        ...
        Member member = repository.findOneByAccountAndStatus(account)
                .orElseThrow(() -> new MemberAccountException("account", ResponseMessages.MEMBER_LOGIN_FAILED));

        // BCrypt 每次 encode 都會產生不同 hash，所以登入時必須用 matches 比對。
        if (!passwordEncoder.matches(request.getPassword(), member.getPassword())) {
            throw new MemberAccountException("password", ResponseMessages.MEMBER_LOGIN_FAILED);
        }
    ...
    public void logout(String refreshToken) {
        ...
        AuthToken authToken = authTokenRepository
                ...
                .orElseThrow(() -> new MemberAccountException("refreshToken", ResponseMessages.TOKEN_INVALID));
    ...
    public void resetPassword(MemberPasswordResetRequest request){
        ...
        if (passwordResetToken.isEmpty()) {
            throw new MemberAccountException("token", ResponseMessages.PASSWORD_RESET_TOKEN_INVALID);
        }
